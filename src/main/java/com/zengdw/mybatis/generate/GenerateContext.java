...
//            introspectedColumn.setTableAlias(tc.getAlias());
            introspectedColumn.setJdbcType(DbToolsUtils.convertTypeNameToJdbcType(column.getDasType().toDataType().typeName, DbToolsUtils.extractDatabaseTypeFromUrl(dbTable)));
            introspectedColumn.setActualTypeName(column.getDasType().toDataType().typeName);
            introspectedColumn.setLength(column.getDasType().toDataType().getLength());
            introspectedColumn.setActualColumnName(column.getName());
            introspectedColumn.setNullable(!column.isNotNull());
            introspectedColumn.setScale(column.getDasType().toDataType().getScale());
